...
//
// Created by tangsilian on 2016-12-19.
//
#include <jni.h>
#include <string>
#include <sys/stat.h>
...
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "tangsilian", __VA_ARGS__)
extern "C" {
...
void verifySign(JNIEnv *env) {
    ...
    //输入签名字符串，这里可以进行相关验证
    LOGE("the signtures is :%s", "tangsilian");
}
...
jstring getDeviceID(JNIEnv *env) {
    ...
    LOGE("the deviceId is %s", "13213513");
    return deviceid;
}
...
void checkAndroidServer() {
       ...
       if ((dirptr = opendir("/data")) != NULL) {  //data目录没有读权限，行不通
        ...
        closedir(dirptr);}
    LOGE("the counts is %s","open data sucess");
